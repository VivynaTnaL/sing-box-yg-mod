> 本分支新增 [三组节点工具与部署说明](chain/README.md)：A 直出、B 直出、A→B 链式，服务器间使用 SS2022，可导入原有五协议接入与聚合文件。原 `sb.sh` 已取消全局关闭防火墙、远程脚本自覆盖及未审计辅助二进制自动下载；请从本地克隆运行。下方保留上游介绍和安装命令，其中远程命令指向上游，不会运行本分支的修复或链式功能。
