> 本分支提供 [独立链式附件与使用说明](chain/ADDON.md)：运行 `bash sb-chain` 打开中文菜单；从标准 sing-box 配置导入，使用独立端口、内核和服务，可与原服务并行。包含 GitHub GeoIP/GeoSite 自动获取和每日更新、客户端国内/局域网直连分流、固定 URL 订阅。`bash sb.sh --generate-config` 打开只生成配置的中文向导；旧模板已拆为独立模块。实现范围与验证记录见 [验收说明](docs/standalone-chain-acceptance.md)。旧接管版说明保留在 [chain/README.md](chain/README.md)。
>
> 原 `sb.sh` 已取消全局关闭防火墙、远程脚本自覆盖及未审计辅助二进制自动下载；请从本地克隆运行。下方保留上游介绍和安装命令，其中远程命令指向上游，不会运行本分支的修复或链式功能。

本分支的原脚本修复范围、验证方法及剩余限制见 [高风险问题修复记录](docs/legacy-fixes.md)。
